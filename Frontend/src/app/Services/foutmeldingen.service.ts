...
export class FoutmeldingenService {
    ...
    let regel:string = regelsVanDezeCursus[i];
        ...
        switch (beschrijving) 
        {
          ...
          default:
            ...
            else foutmeldingen.push("De " + i + "e beschrijving hoort niet in een cursus:" + beschrijving + ".")
            break;
        }
  ...
  CheckDuur(inhoud: string, foutmeldingen : string[], teller : number) : string[]
  {
    ...
    {
      foutmeldingen.push("De opbouw van de duur \"" + inhoud + "\" van de " + (teller+1) + "e cursus voldoet niet aan de verwachting")
    }
    if(!+getalKandidaat)
    {
      foutmeldingen.push("Het eerste deel van de duur van de " + (teller+1) + "e cursus is : \"" + inhoud +"\" ,niet een getal")
    }
    if(!dagenKandidaat){
      foutmeldingen.push("De beschrijving van de duur van cursus " + (teller+1) + "e cursus ontbreekt.")
    }
    else if(dagenKandidaat.trim() !== "dagen")
    {
      foutmeldingen.push(("De beschrijving van de " + (teller+1) + "e cursus is: \"" + inhoud + "\", en niet \"dagen\""))
    }
    return foutmeldingen;
  }
